...
class TestAccountService(TestCase):
    ...
    def test_list_accounts(self):
        """Test to ensure that we can fetch a correct list of all accounts"""
    ...
    def test_update_accounts(self):
        """Test to ensure we can update a pre-existing account"""
        test_account = AccountFactory()
        response = self.client.post(BASE_URL, json=test_account.serialize())
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        new_account = response.get_json()
        new_account["name"] = "Fake Name Which Is Unique"

        response = self.client.put(BASE_URL + f"/{new_account['id']}", json=new_account)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_delete_accounts(self):
        """Test to ensure that we can delete accounts through the REST API"""
        account = self._create_accounts(1)[0]
    ...
    def test_headers_contain_cors_policy(self):
        """Test to ensure that requests return a CORS header"""
        response = self.client.get("/", environ_overrides=HTTPS_ENVIRON)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
